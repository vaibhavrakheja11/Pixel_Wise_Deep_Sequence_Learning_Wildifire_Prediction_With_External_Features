...
#load data
test_dir = 'Data_TIFF/test_div'
train_dir = 'Data_TIFF/train_div'
val_dir = 'Data_TIFF/eval_div'
...
def my_generator(batch_size, img_dir):
    ...
    while True:
        input_images = np.zeros(
            (batch_size, config['width'], config['height'], 1 * 5))
...
model = Sequential()
model.add(Conv2D(1, (3, 3), activation='relu', padding='same', input_shape=(config['height'], config['width'], 5 * 1)))
...
for i, each_dir in enumerate(cat_dirs):
    input_images = np.zeros(
            (1, config['width'], config['height'], 1 * 5))
...
reshaped = Reshape((128,128,5,1))(inp)
permuted = Permute((1,2,4,3))(reshaped)
noise = GaussianNoise(0.1)(permuted)
last_layer = Lambda(slice, input_shape=(128,128,1,5), output_shape=(128,128,1))(noise)
